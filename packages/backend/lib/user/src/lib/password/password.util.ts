...
function splitAndGet(formattedPassword: string, index: number): string | null {
  if (isEmpty(formattedPassword)) {
    return null;
  }

  if (
    !formattedPassword.startsWith(PREFIX) ||
    !formattedPassword.endsWith(POSTFIX)
  ) {
    return null;
  }

  const values = formattedPassword.substring(
    PREFIX.length,
    formattedPassword.length - POSTFIX.length
  );
